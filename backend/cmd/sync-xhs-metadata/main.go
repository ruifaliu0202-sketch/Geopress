...
func main() {
	...
	if out == "" || dbg == "" {
		defaultDebugDir := filepath.Join(defaultRuntimeRoot(profile), "runtime", "xhs-metadata-debug", *workspaceID, *accountID)
		if out == "" {
	...
	job := model.MediaAccountSyncJob{
		ID:                fmt.Sprintf("masj_xhs_meta_%d", now.UnixNano()),
		...
		RequestedAt:       now,
		StartedAt:         &now,
		IdempotencyKey:    fmt.Sprintf("xhs-metadata:%s", now.Format("20060102150405")),
		RequestPayload:    map[string]any{"profileDir": profile, "metadataUrl": xiaohongshu.DefaultAccountMetadataURL},
	...
	collector := xiaohongshu.NewPlaywrightAccountMetadataCollector()
	result, collectErr := collector.Collect(ctx, xiaohongshu.AccountMetadataCollectRequest{
	...
	if collectErr != nil {
		...
		log.Fatalf("collect xiaohongshu metadata: %v", collectErr)
	}
	...
	job.ResultSummary = map[string]any{
		...
		"redAccountId":           result.Metadata.RedAccountID,
		"followerCount":          result.Metadata.FollowerCount,
		"followingCount":         result.Metadata.FollowingCount,
		"likedAndFavoritedCount": result.Metadata.LikedAndFavoritedCount,
...
func buildSyncResult(account model.MediaAccount, result xiaohongshu.AccountMetadataCollectResult, profileDir string, requestedAt time.Time) (model.MediaAccount, model.MediaAccountMetricSnapshot) {
	capturedAt := result.CapturedAt
	...
	metadata["platformMetadata"] = map[string]any{
		...
		"accountStatusAlt": result.Metadata.AccountStatusAlt,
		"profileBaseText":  result.Metadata.ProfileBaseText,
		"accountCardText":  result.Metadata.AccountCardText,
	...
	account.ExternalID = firstNonEmpty(result.Metadata.RedAccountID, account.ExternalID)
	...
	account.LastSyncMessage = "xiaohongshu metadata sync completed"

	rawMetrics := map[string]any{
		"accountStats":           result.Metadata.AccountStats,
		"overviewMetrics":        result.Metadata.OverviewMetrics,
		"likedAndFavoritedCount": result.Metadata.LikedAndFavoritedCount,
		"collectorStatus":        result.Status,
		"collectorSelectors":     result.Selectors,
		"rawRedAccountText":      result.Metadata.RawRedAccountText,
	}
	...
	snapshot := model.MediaAccountMetricSnapshot{
		ID:                 fmt.Sprintf("mams_xhs_%s_%d", account.ID, capturedAt.UnixNano()),
		WorkspaceID:        account.WorkspaceID,
		MediaAccountID:     account.ID,
		PlatformID:         account.PlatformID,
		Source:             "browser_visible_dom",
		CapturedAt:         capturedAt,
		FollowerCount:      result.Metadata.FollowerCount,
		FollowingCount:     result.Metadata.FollowingCount,
		TotalLikeCount:     result.Metadata.LikedAndFavoritedCount,
