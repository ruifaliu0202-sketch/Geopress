...
func (db *DB) SaveBrandAsset(ctx context.Context, item model.BrandAsset) error {
	...
	channels, err := brandJSONText(item.Channels, []string{})
	if err != nil {
		return err
	}
	tags, err := brandJSONText(item.Tags, []string{})
	if err != nil {
		return err
	}
	metadata, err := brandJSONText(item.Metadata, map[string]string{})
	if err != nil {
		return err
	}
...
func (db *DB) SaveBrandGuardrail(ctx context.Context, item model.BrandGuardrail) error {
	...
	rules, err := brandJSONText(item.Rules, []string{})
	if err != nil {
		return err
	}
	createdAt := defaultTime(item.CreatedAt)
...
func saveApprovalWorkflowExec(ctx context.Context, exec sqlExecutor, item model.ApprovalWorkflow) error {
	stages, err := brandJSONText(item.Stages, []model.ApprovalStage{})
	if err != nil {
		return err
	}
	createdAt := defaultTime(item.CreatedAt)
...
func (db *DB) SaveAgencyClientRelation(ctx context.Context, item model.AgencyClientRelation) error {
	...
	scopes, err := brandJSONText(item.Scopes, []string{})
	if err != nil {
		return err
	}
	createdAt := defaultTime(item.CreatedAt)
...
func (db *DB) loadReportPackages(ctx context.Context) ([]model.ReportPackage, error) {
	...
	for rows.Next() {
		...
		item.Metrics = decodeBrandAnyMap(metrics)
		items = append(items, item)
	}
	return items, rows.Err()
}
...
func (db *DB) SaveReportPackage(ctx context.Context, item model.ReportPackage) error {
	...
	sections, err := brandJSONText(item.Sections, []string{})
	if err != nil {
		return err
	}
	metrics, err := brandJSONText(item.Metrics, map[string]any{})
	if err != nil {
		return err
	}
	createdAt := defaultTime(item.CreatedAt)
...
func (db *DB) SaveStrategyRecommendation(ctx context.Context, item model.StrategyRecommendation) error {
	...
	evidence, err := brandJSONText(item.Evidence, []string{})
	if err != nil {
		return err
	}
	createdAt := defaultTime(item.CreatedAt)
...
func brandJSONText(value any, fallback any) (string, error) {
	if value == nil {
		value = fallback
	}
...
func decodeBrandAnyMap(value string) map[string]any {
	if value == "" {
