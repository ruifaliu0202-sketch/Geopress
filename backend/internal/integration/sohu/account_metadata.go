...
const PlatformType = "sohu"
const DefaultAccountMetadataURL = "https://mp.sohu.com/mpfe/v4/"
...
type AccountMetadataSnapshot struct {
	...
	RegisterStatus     *float64       `json:"registerStatus"`
	SecurityScore      *float64       `json:"securityScore"`
	RiskLevel          *float64       `json:"riskLevel"`
	NewUser            bool           `json:"newUser"`
	Admin              bool           `json:"admin"`
	HasOperator        bool           `json:"hasOperator"`
	UserIsOperator     bool           `json:"userIsOperator"`
	OrgStatusName      string         `json:"orgStatusName"`
	LocationName       string         `json:"locationName"`
	ProvinceName       string         `json:"provinceName"`
	CityName           string         `json:"cityName"`
	VisibleText        string         `json:"visibleText"`
	VisibleAccountName string         `json:"visibleAccountName"`
	RightsSummary      map[string]any `json:"rightsSummary"`
	CommonAuth         map[string]any `json:"commonAuth"`
	UserCheck          map[string]any `json:"userCheck"`
	AccountInfo        map[string]any `json:"accountInfo"`
	ListAccount        map[string]any `json:"listAccount"`
	RegisterInfo       map[string]any `json:"registerInfo"`
}
...
func NewPlaywrightAccountMetadataCollector() PlaywrightAccountMetadataCollector {
	return PlaywrightAccountMetadataCollector{
		...
		ActionTimeout: defaultDurationSeconds("GEOPRESS_SOHU_METADATA_TIMEOUT_SECONDS", 90*time.Second),
	}
}
...
func (c PlaywrightAccountMetadataCollector) Collect(ctx context.Context, req AccountMetadataCollectRequest) (AccountMetadataCollectResult, error) {
	...
	if err := cmd.Run(); err != nil {
		...
		if decodeErr := json.Unmarshal(bytes.TrimSpace(stdout.Bytes()), &result); decodeErr == nil && result.Status != "" {
			...
			return result, fmt.Errorf("sohu account metadata status: %s", defaultString(result.Status, "unknown"))
		}
		...
		return AccountMetadataCollectResult{}, fmt.Errorf("sohu account metadata collection failed: %s", message)
	}

	var result AccountMetadataCollectResult
	if err := json.Unmarshal(bytes.TrimSpace(stdout.Bytes()), &result); err != nil {
		return AccountMetadataCollectResult{}, fmt.Errorf("decode sohu metadata output: %w", err)
	}
	if result.ProfileDir == "" {
		result.ProfileDir = profileDir
	}
...
func defaultAccountMetadataScriptPath() string {
	if value := strings.TrimSpace(os.Getenv("GEOPRESS_SOHU_ACCOUNT_METADATA_SCRIPT")); value != "" {
		return value
	}
	return filepath.Join(installRoot(), "scripts", "sohu-account-metadata.mjs")
}
